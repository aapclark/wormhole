...
func GuardianOptionQueryHandler(ccqEnabled bool, stakingEnabled bool, ethRPCURL string, ccqFactoryAddress string, ipfsGateway string) *GuardianOption {
	return &GuardianOption{
		name: "query",
		f: func(ctx context.Context, logger *zap.Logger, g *G) error {
			if !ccqEnabled {
			...
			// NOTE: Rate limiting is now enforced by the CCQ server before requests reach guardians.
			// Guardians trust that requests on the gossip network have already been validated.
			// This aligns with the CCQ whitepaper design.
			logger.Info("ccq: cross chain query is enabled - rate limiting enforced by CCQ server", zap.String("component", "ccq"))

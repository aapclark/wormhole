...
describe("Delegation Integration Tests", () => {
  ...
  test("Address with no delegation and no stake cannot query", async () => {
    ...
    const queryRequest = new QueryRequest(getCurrentTimestampSeconds(), [
      perChainQuery,
    ]);
  ...
  test("V1 format (no stakerAddress) still works for self-staking", async () => {
    const delegator = getNextDelegator();
  ...
  test("V2 format includes stakerAddress in serialization", async () => {
    const delegator = getNextDelegator();

...
export async function ensureStakerHasStake(
  ...
  if (BigInt(stakeInfo[0]) >= stakeAmountWei) {
    return;
  }
  ...
  if (balance < stakeAmountWei) {
    throw new Error(
      `Insufficient token balance. Need ${stakeAmountTokens} tokens, have ${formatEther(
        balance
      )} tokens`
    );
  }

  const neededAmount = stakeAmountWei - BigInt(stakeInfo[0]);
  ...
  if (neededAmount > BigInt(0)) {
    ...
    if (ethBalance === BigInt(0)) {
      ...
      const hash = await minterClient.sendTransaction({
        to: stakerAddress,
        value: parseEther("1"),
      } as any);
      await minterClient.waitForTransactionReceipt({ hash });
    }
...
export async function sendSolanaQuery(
  ...
  const request = new QueryRequest(nonce, queries, undefined, stakerAddress);
  const serialized = request.serialize();
  const digest = QueryRequest.digest(network, serialized);
  const signature = sign(privateKey.slice(2), digest);
